...
function check_network() {
    ...
    if ping -c 1 8.8.8.8 &>/dev/null; then
        ...
        echo "网络连接异常。请检查网络设置后重新运行脚本。"
        exit 1
    fi
}

# 安装中文字体（以文泉驿微米黑字体为例，可按需替换）
...
function change_mirror_source() {
    ...
    mirrorlist_content+="Server = http://mirrors.jlu.edu.cn/archlinux/\$repo/os/\$arch\n"
    mirrorlist_content+="Server = https://mirrors.aliyun.com/archlinux/\$repo/os/\$arch\n"
    mirrorlist_content+="Server = https://mirrors.ustc.edu.cn/archlinux/\$repo/os/\$arch\n"
    mirrorlist_content+="Server = https://mirrors.wsyu.edu.cn/archlinux/\$repo/os/\$arch\n"
    # ... 其他镜像源地址 ...
...
# 磁盘分区操作（这里只是引导用户进入cfdisk进行分区，后续获取分区信息用于后续安装流程）
function disk_partition() {
    echo "即将进入磁盘分区操作，我不提供具体的分区操作方法，你需要自行使用cfdisk工具进行分区。"
    echo "以下是当前系统识别到的磁盘设备列表："
    lsblk
    ...
    # 调用cfdisk让用户进行分区操作
    cfdisk $disk_device
    # 获取分区后的磁盘设备根分区、交换分区和EFI分区信息
    read -p "请输入分区后的根分区设备名称（例如 /dev/sda3）：" root_partition
    read -p "请输入分区后的交换分区设备名称（例如 /dev/sda2）：" swap_partition
    read -p "请输入分区后的EFI分区设备名称（例如 /dev/sda1）：" efi_partition
    echo "磁盘分区操作完成，以下是你输入的分区信息："
    echo "根分区设备：$root_partition"
    echo "交换分区设备：$swap_partition"
    echo "EFI分区设备：$efi_partition"
    # 将分区信息传递回主函数，后续用于安装流程
    echo $root_partition > /tmp/root_partition.txt
    echo $swap_partition > /tmp/swap_partition.txt
    echo $efi_partition > /tmp/efi_partition.txt
}

# 安装Arch Linux基础步骤（简化示意，实际需严格参照官方完整流程，适配UEFI引导）
function install_archlinux() {
    echo "开始Arch Linux安装流程，以下操作基于你之前进行的磁盘分区，请谨慎操作，可能导致数据丢失"
    # 获取之前记录的分区信息
    root_partition=$(cat /tmp/root_partition.txt)
    swap_partition=$(cat /tmp/swap_partition.txt)
    efi_partition=$(cat /tmp/efi_partition.txt)
    ...
    # 安装引导程序（适配UEFI，以GRUB为例，不同硬件平台需相应调整）
    ...
    echo "Arch Linux安装完成，可以重启电脑进入新系统。"
}
